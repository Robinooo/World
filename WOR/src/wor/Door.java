...
import java.util.*;
/**
 * The class Door is used to create a door. A door have a 
 * name, can have a lock or a code (not both in the same time).
 * A door can also be open or not, if the user have a key or code, 
 * or if the door do not have a lock (code or key), or is already unlock.
 *
 * @author Group2
 * @version 11/20/2017
 */
public class Door
{
   ...
   * At first, the door is close and have a specifi name.
   * ifKeyLock is true then, the door have a key lock, else not.
   * ifCodeLock is true then, the door have a code lock, else not.
   * A door can not have a code and a key lock
   * 
   ...
   * 
   * @param b The boolean to know if a door is openable or not. When the value is true -> the door is openable (can be open)
   */
   ...
   */
   ...
   /** 
   * Open the door using a passcode
   * The method call returns 'true' if this map 
   ...
   * 
   * @return Returns false when a door can't be open. The door still lock. And return true when the door can be passed.
   */
